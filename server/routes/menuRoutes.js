...
router.post('/', authMiddleware, (req, res) => {
  ...
  try {
    ...
    const result = db.prepare('INSERT INTO menus (user_id, name, slug, restaurant_name, tagline, accent_color, description) VALUES (?,?,?,?,?,?,?)').run(req.user.id, name, slug, restaurant_name || '', tagline || '', accent_color || '#C8622A', description || '');
...
router.put('/:id', authMiddleware, (req, res) => {
  ...
  try {
    const menu = db.prepare('SELECT * FROM menus WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
    if (!menu) return res.status(404).json({ error: 'Menu not found' });
    db.prepare('UPDATE menus SET name=?, restaurant_name=?, tagline=?, accent_color=?, description=?, updated_at=CURRENT_TIMESTAMP WHERE id=?').run(name || menu.name, restaurant_name ?? menu.restaurant_name, tagline ?? menu.tagline, accent_color || menu.accent_color, description ?? menu.description, menu.id);
