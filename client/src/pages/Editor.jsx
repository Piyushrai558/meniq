      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div className="editor-sidebar">
          ...
          <ul className="section-list">
            {menu.sections.map(s => (
              <li key={s.id} className={s.id === activeSectionId ? 'active' : ''} onClick={() => { setActiveSectionId(s.id); setShowAddForm(false); }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div>
              ...
              <input type="color" value={accentColor} onChange={e => setAccentColor(e.target.value)} style={{ width: '100%', height: 36, border: '1px solid var(--border-strong)', borderRadius: 'var(--radius-sm)', cursor: 'pointer', padding: '2px 4px' }} />
            </div>
            <button className="btn-sm btn-sm-accent" style={{ marginTop: 8 }} onClick={saveSettings}>Save settings</button>
          </div>
        </div>
        ...
        <div className="editor-main">
          <div className="editor-top">
            ...
            <div className="editor-actions">
              <button className="btn-sm btn-sm-outline" onClick={() => window.open(`/menu/${menu.slug}`, '_blank')}>Preview live</button>
            </div>
          </div>

          <div className="items-list">
            {items.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 40, color: 'var(--ink-faint)', fontSize: 14 }}>No items yet — add your first!</div>
            ) : items.map(item => (
              <div className="item-card" key={item.id}>
                <div className="item-thumb">{item.emoji || (item.type === 'nonveg' ? '🍗' : '🥦')}</div>
                <div className="item-info">
                  <div className="item-name">
                    {item.name}
                    <span className={`item-tag ${item.type === 'veg' ? 'tag-veg' : 'tag-nonveg'}`}>{item.type === 'veg' ? 'Veg' : 'Non-veg'}</span>
                ...
                <div className="item-controls">
                  <div className="toggle-wrap">
                    ...
                    <div className={`toggle ${item.is_active ? 'on' : ''}`} onClick={() => toggleItem(item.id, item.is_active ? 0 : 1)}>
            ...
            {showAddForm ? (
              <div className="add-item-form">
                ...
                <div className="form-row">
                  <div className="form-group-sm"><label>Item name</label><input className="form-input-sm" placeholder="e.g. Paneer Tikka" value={newItem.name} onChange={e => setNewItem(p => ({ ...p, name: e.target.value }))} /></div>
                  <div className="form-group-sm"><label>Price (₹)</label><input className="form-input-sm" placeholder="250" type="number" value={newItem.price} onChange={e => setNewItem(p => ({ ...p, price: e.target.value }))} /></div>
                </div>
                <div className="form-group-sm"><label>Description</label><input className="form-input-sm" placeholder="Short description..." value={newItem.description} onChange={e => setNewItem(p => ({ ...p, description: e.target.value }))} /></div>
                <div className="form-group-sm">
                  <label>Tags</label>
                  <div className="tag-select">
                    {['veg', 'nonveg', 'spicy', 'bestseller'].map(tag => (
                      <button key={tag} className={`tag-option ${selectedTags.includes(tag) ? `selected-${tag}` : ''}`} onClick={() => toggleTag(tag)}>
                ...
                <div className="form-actions-sm">
                  <button className="btn-sm btn-sm-outline" onClick={() => setShowAddForm(false)}>Cancel</button>
                  <button className="btn-sm btn-sm-accent" onClick={handleAddItem}>Add item</button>
                </div>
              </div>
            ) : (
              <button className="btn-add-section" style={{ width: '100%', marginTop: 12 }} onClick={() => setShowAddForm(true)}>
